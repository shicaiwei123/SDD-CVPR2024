...
sys.path.append('/home/bbb/shicaiwei/mddstillar_origin/mdistiller')
...
class RCNNKD(nn.Module):
    ...
    def __init__(
        ...
        # self.roi_heads.box_head.fc1 = nn.Linear(12544, 1024, bias=True)
        # self.teacher.roi_heads.box_head.fc1 = nn.Linear(12544, 1024, bias=True)
        self.patch_num = 5

        # if self.kd_args.TYPE == "GL_DKD":
        #
        #     pooler_scales=[0.25,0.125,0.0625,0.03125]
        #     self.roi_heads.box_pooler= ROIPooler(
        #         output_size=14,
        ...
        #     )
        #
        #     self.teacher.roi_heads.box_pooler= ROIPooler(
        #         output_size=14,
        #         scales=pooler_scales,
        #         sampling_ratio=0,
        #         pooler_type='ROIAlignV2',
        ...
        self.x1 = [0, 2, 4, 6, 8, 10, 12]
        self.x2 = [1, 3, 5, 7, 9, 11, 13]
        self.mask1 = torch.zeros((14, 14), dtype=torch.bool)
        self.mask2 = torch.zeros((14, 14), dtype=torch.bool)
        self.mask3 = torch.zeros((14, 14), dtype=torch.bool)
        self.mask4 = torch.zeros((14, 14), dtype=torch.bool)
    ...
    def forward_pure_roi_head(self, roi_head, features, proposals):
        ...
        box_features = self.pooling2(features, [x.proposal_boxes for x in proposals])

        if self.kd_args.TYPE == "GL_DKD":
            box_features_0 = self.pooling(box_features)
            box_features_1 = torch.masked_select(box_features, self.mask1)
            box_features_1 = torch.reshape(box_features_1, (1024, 256, 7, 7))

            box_features_2 = torch.masked_select(box_features, self.mask2)
            box_features_2 = torch.reshape(box_features_2, (1024, 256, 7, 7))

            box_features_3 = torch.masked_select(box_features, self.mask3)
            box_features_3 = torch.reshape(box_features_3, (1024, 256, 7, 7))

            box_features_4 = torch.masked_select(box_features, self.mask4)
            box_features_4 = torch.reshape(box_features_4, (1024, 256, 7, 7))

            box_features = torch.cat((box_features_0, box_features_1, box_features_2, box_features_3, box_features_4),
                                     dim=0)
        else:
            box_features = self.pooling(box_features)
    ...
    def forward(self, batched_inputs: Tuple[Dict[str, torch.Tensor]]):
        ...
        elif self.kd_args.TYPE == "GL_DKD":
            # print(1111)
            teacher_images = self.teacher_preprocess_image(batched_inputs)
            t_features = self.teacher.backbone(teacher_images.tensor)
            stu_predictions = self.forward_pure_roi_head(self.roi_heads, features, sampled_proposals)
            stu_predictions_0 = torch.reshape(stu_predictions[0], (self.patch_num, 1024, 81))
            stu_predictions_1 = torch.reshape(stu_predictions[1], (self.patch_num, 1024, 320))

            tea_predictions = self.forward_pure_roi_head(self.teacher.roi_heads, t_features, sampled_proposals)
            tea_predictions_0 = torch.reshape(tea_predictions[0], (self.patch_num, 1024, 81))
            tea_predictions_1 = torch.reshape(tea_predictions[1], (self.patch_num, 1024, 320))

            dkd_sum = torch.tensor(0.0).cuda()
            count_num = 5
            for i in range(self.patch_num):
                stu_predictions = tuple((stu_predictions_0[i, :, :], stu_predictions_1[i, :, :]))
                tea_predictions = tuple((tea_predictions_0[i, :, :], tea_predictions_1[i, :, :]))
                dkd_loss_one = rcnn_dkd_loss(
                    stu_predictions, tea_predictions, [x.gt_classes for x in sampled_proposals],
                    self.kd_args.DKD.ALPHA, self.kd_args.DKD.BETA, self.kd_args.DKD.T)
                if dkd_loss_one['loss_dkd'].isinf():
                    count_num = count_num - 1
                    continue
                dkd_sum += dkd_loss_one['loss_dkd']
            detector_losses.update({'loss_dkd': dkd_sum / count_num})
